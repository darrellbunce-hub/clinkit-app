...
| **Implementation consequence** | Account UI already surfaces grace. **No** Keynetic payment-failure/grace email template exists yet — implementation backlog. Do not claim Keynetic emails this until shipped. Stripe-hosted receipts remain separate. |
...
| ID | Item | Notes |
|----|------|-------|
| BL-01 | Payment-failure / grace customer email (D-B4) | No template today; use Resend communications pattern when built |
